...
export default function Invitation({ includeHenna = false }: { includeHenna?: boolean }) {
  const celebrations = includeHenna ? [henna, wedding] : [wedding];
  const calendar = includeHenna ? '/nouhaila-issam.ics' : '/wedding/invitation.ics';
  const title = 'دعوة زفاف نهيلة وعصام';
    ...
    <MotionExperience>
      ...
      <main id="home">
        ...
        <section className="celebrations section-pad" id="celebrations" aria-labelledby="celebration-heading">
          ...
          <div className={`event-grid${includeHenna ? '' : ' event-grid-single'}`}>
          ...
          <a className="calendar-link" href={calendar} download="nouhaila-issam.ics" data-reveal>
            <CalendarDays size={19} strokeWidth={1.4} aria-hidden="true" />{includeHenna ? 'احفظوا الموعدين في التقويم' : 'احفظوا الموعد في التقويم'}
          </a>
        </section>
      ...
      <footer className="site-footer" data-reveal>
        ...
        <a className="footer-names" href="#home" aria-label="العودة إلى بداية الدعوة">
          نهيلة <span>و</span> عصام
        </a>
